				...
				hbService.getCharacteristic(Characteristic.CurrentRelativeHumidity)
					.on('get', async (callback) => {
						try {
							...
							if(client.cachedValue && client.cachedValue[characteristic])
							{
								this.log('[get] Humidity 1' + client.accessoryNames[characteristic] + ' cachedValue ' + characteristic + ' = ' + client.cachedValue[characteristic] + ' => ok')
								callback(null, client.cachedValue[characteristic])
								callbacked = true
								return
							}

                            console.log(client.getServices())
							const result = await client.getCharacteristics([ characteristic])
							...
							this.log('[get] Humidity 2' + client.accessoryNames[characteristic] + ' getCharacteristics ' + characteristic + ' = ' + result.characteristics[0].value + ' => ok')
							...
							callback(false);
							this.log('[get] Humidity 3' + client.accessoryNames[characteristic] + ' getCharacteristics ' + characteristic + ' => error', e)
						}
					});
